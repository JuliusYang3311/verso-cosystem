...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("verso-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("verso-bin-sherpa-");
      const modelDir = await makeTempDir("verso-sherpa-model-");
      ...
      const cfg: VersoConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("verso-bin-whispercpp-");
      const modelDir = await makeTempDir("verso-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("verso-bin-gemini-");
      ...
      const cfg: VersoConfig = { tools: { media: { image: {} } } };
