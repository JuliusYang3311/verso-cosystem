...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `verso-e2e-${name}-`));
  const configDir = path.join(homeDir, ".verso");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          VERSO_GATEWAY_TOKEN: "",
          VERSO_GATEWAY_PASSWORD: "",
          VERSO_SKIP_CHANNELS: "1",
          VERSO_SKIP_BROWSER_CONTROL_SERVER: "1",
          VERSO_SKIP_CANVAS_HOST: "1",
        },
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        VERSO_GATEWAY_TOKEN: inst.gatewayToken,
        VERSO_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          VERSO_GATEWAY_PORT: String(gwA.port),
          VERSO_GATEWAY_TOKEN: gwA.gatewayToken,
          VERSO_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          VERSO_GATEWAY_PORT: String(gwB.port),
          VERSO_GATEWAY_TOKEN: gwB.gatewayToken,
          VERSO_GATEWAY_PASSWORD: "",
        }),
