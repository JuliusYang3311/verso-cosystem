...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    stateDir: process.env.VERSO_STATE_DIR,
  };
}
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("VERSO_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.VERSO_STATE_DIR = path.join(base, ".verso");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "verso-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".verso", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
