  ...
  mainWindow.on('close', (event) => {
    if (!app.isQuitting) {
      ...
      if (mainWindow.isFullScreen()) {
        // Exit fullscreen first, then hide — avoids macOS black screen artifact
...
ipcMain.handle('save-config', async (event, updates) => {
  ...
  // Keys listed in _replaceKeys are set atomically (not deep-merged) — used for
  // collections like providers where deletion must propagate.
