...
This is an **alpha** iOS app that connects to an Verso Gateway as a `role: node`.
...
3. On iOS: Verso → Settings → Gateway → paste setup code → Connect
...
## Shared packages
- `../shared/VersoKit` — shared types/constants used by iOS (and later macOS bridge + gateway routing).

- `apps/shared/VersoKit` contains the shared transport/types used by the iOS app.
