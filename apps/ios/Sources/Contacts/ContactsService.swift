...
final class ContactsService: ContactsServicing {
    ...
    func search(params: VersoContactsSearchParams) async throws -> VersoContactsSearchPayload {
        ...
        return VersoContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: VersoContactsAddParams) async throws -> VersoContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return VersoContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return VersoContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> VersoContactPayload {
        VersoContactPayload(
