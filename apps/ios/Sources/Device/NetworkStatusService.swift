...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> VersoNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> VersoNetworkStatusPayload {
        let status: VersoNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> VersoNetworkStatusPayload {
        VersoNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
