...
struct IOSGatewayChatTransport: VersoChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> VersoChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(VersoChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> VersoChatHistoryPayload {
        ...
        return try JSONDecoder().decode(VersoChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [VersoChatAttachmentPayload]) async throws -> VersoChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [VersoChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(VersoChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(VersoGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: VersoGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: VersoChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: VersoAgentEventPayload.self)
                        {
