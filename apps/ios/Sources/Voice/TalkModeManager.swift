...
final class TalkModeManager: NSObject {
    ...
    private var pttCompletion: CheckedContinuation<VersoTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = VersoTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> VersoTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return VersoTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> VersoTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> VersoTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return VersoTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> VersoTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = VersoTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: VersoTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: VersoAgentEventPayload.self) else {
                continue
            }
