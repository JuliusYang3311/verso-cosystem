...
final class RemindersService: RemindersServicing {
    func list(params: VersoRemindersListParams) async throws -> VersoRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[VersoReminderPayload], Error>) in
        ...
        return VersoRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: VersoRemindersAddParams) async throws -> VersoRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = VersoReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return VersoRemindersAddPayload(reminder: payload)
    }
