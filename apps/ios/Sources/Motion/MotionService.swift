...
final class MotionService: MotionServicing {
    func activities(params: VersoMotionActivityParams) async throws -> VersoMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[VersoMotionActivityEntry], Error>) in
        ...
        return VersoMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: VersoPedometerParams) async throws -> VersoPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<VersoPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = VersoPedometerPayload(
