...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [VersoCapability.canvas.rawValue, VersoCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(VersoCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(VersoCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = VersoLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(VersoCapability.location.rawValue) }

        caps.append(VersoCapability.device.rawValue)
        caps.append(VersoCapability.photos.rawValue)
        caps.append(VersoCapability.contacts.rawValue)
        caps.append(VersoCapability.calendar.rawValue)
        caps.append(VersoCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(VersoCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            VersoSystemCommand.notify.rawValue,
            VersoChatCommand.push.rawValue,
        ...
        if caps.contains(VersoCapability.camera.rawValue) {
            commands.append(VersoCameraCommand.list.rawValue)
            commands.append(VersoCameraCommand.snap.rawValue)
            commands.append(VersoCameraCommand.clip.rawValue)
        }
        if caps.contains(VersoCapability.location.rawValue) {
            commands.append(VersoLocationCommand.get.rawValue)
        }
        if caps.contains(VersoCapability.device.rawValue) {
            commands.append(VersoDeviceCommand.status.rawValue)
            commands.append(VersoDeviceCommand.info.rawValue)
        }
        if caps.contains(VersoCapability.photos.rawValue) {
            commands.append(VersoPhotosCommand.latest.rawValue)
        }
        if caps.contains(VersoCapability.contacts.rawValue) {
            commands.append(VersoContactsCommand.search.rawValue)
            commands.append(VersoContactsCommand.add.rawValue)
        }
        if caps.contains(VersoCapability.calendar.rawValue) {
            commands.append(VersoCalendarCommand.events.rawValue)
            commands.append(VersoCalendarCommand.add.rawValue)
        }
        if caps.contains(VersoCapability.reminders.rawValue) {
            commands.append(VersoRemindersCommand.list.rawValue)
            commands.append(VersoRemindersCommand.add.rawValue)
        }
        if caps.contains(VersoCapability.motion.rawValue) {
            commands.append(VersoMotionCommand.activity.rawValue)
            commands.append(VersoMotionCommand.pedometer.rawValue)
        }
