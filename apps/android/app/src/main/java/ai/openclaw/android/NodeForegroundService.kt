...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    super.onCreate()
    ensureChannel()
    val initial = buildNotification(title = "Verso Node", text = "Starting…")
    ...
    notificationJob =
      scope.launch {
        combine(
          ...
          val title = if (connected) "Verso Node · Connected" else "Verso Node"
          val voiceSuffix =
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = "Verso node connection status"
        setShowBadge(false)
      }
