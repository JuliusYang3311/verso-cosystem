...
enum VersoLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = VersoOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = VersoFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        VersoLogging.bootstrapIfNeeded()
        let label = VersoLogging.makeLabel(subsystem: subsystem, category: category)
...
struct VersoOSLogHandler: LogHandler {
...
struct VersoFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = VersoLogging.parseLabel(self.label)
