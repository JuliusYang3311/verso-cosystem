...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: VersoOAuthStore.AnthropicOAuthStatus = VersoOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: VersoOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = VersoOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = VersoOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try VersoOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Verso can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: VersoOAuthStore.AnthropicOAuthStatus,
