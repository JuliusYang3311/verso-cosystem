...
extension OnboardingView {
    ...
    func refreshAnthropicOAuthStatus() {
        _ = VersoOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        let previous = self.anthropicAuthDetectedStatus
        let status = VersoOAuthStore.anthropicOAuthStatus()
    ...
    func verifyAnthropicOAuthIfNeeded(force: Bool = false) async {
        ...
        guard let refresh = VersoOAuthStore.loadAnthropicOAuthRefreshToken(), !refresh.isEmpty else {
        ...
        do {
            ...
            try VersoOAuthStore.saveAnthropicOAuth(updated)
            self.refreshAnthropicOAuthStatus()
