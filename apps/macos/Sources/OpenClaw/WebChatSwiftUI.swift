...
struct MacGatewayChatTransport: VersoChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> VersoChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(VersoChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> VersoChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                VersoGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<VersoChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any VersoChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = VersoChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: VersoChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<VersoChatView>) -> NSViewController
    {
