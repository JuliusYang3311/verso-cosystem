...
enum CommandResolver {
    ...
    static func projectVersoExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func hasAnyVersoInvoker(searchPaths: [String]? = nil) -> Bool {
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let openclawPath = self.projectVersoExecutable(projectRoot: root) {
    ...
    {
        let root = configRoot ?? VersoConfigFile.loadDict()
