...
final class PeekabooBridgeHostCoordinator {
    ...
    private var services: VersoPeekabooBridgeServices?
    private static var openclawSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Verso", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = VersoPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: PeekabooBridgeConstants.versoSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(PeekabooBridgeConstants.versoSocketPath, privacy: .public)")
    }
...
@MainActor
private final class VersoPeekabooBridgeServices: PeekabooBridgeServiceProviding {
