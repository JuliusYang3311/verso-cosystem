...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.verso/moltbot.json. Prefer CLAWDBOT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.verso/moltbot.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.verso/moltbot.json. Restart the gateway to apply."
        }
