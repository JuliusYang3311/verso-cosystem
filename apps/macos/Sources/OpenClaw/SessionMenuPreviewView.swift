...
enum SessionMenuPreviewLoader {
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> VersoSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: VersoSessionPreviewEntry,
    ...
    private static func cache(payload: VersoSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: VersoChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [VersoKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [VersoKit.AnyCodable]) -> [VersoChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(VersoChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: VersoChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: VersoChatMessage) -> Bool {
    ...
    private static func toolNames(for message: VersoChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: VersoChatMessage) -> String? {
