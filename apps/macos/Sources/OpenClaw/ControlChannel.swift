...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: VersoProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: VersoKit.AnyCodable]()) {
                $0[$1.key] = VersoKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: VersoProtocol.AnyCodable?) -> [String: VersoProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: VersoProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: VersoKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: VersoProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
