...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: VersoProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: VersoProtocol.AnyCodable] = [
            "presence": .init([VersoProtocol.AnyCodable(entry)]),
        ]
        let payload = VersoProtocol.AnyCodable(payloadMap)
