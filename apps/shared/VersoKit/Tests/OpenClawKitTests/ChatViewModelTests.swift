...
private final class TestChatTransport: @unchecked Sendable, VersoChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [VersoChatHistoryPayload]
    private let sessionsResponses: [VersoChatSessionsListResponse]

    private let stream: AsyncStream<VersoChatTransportEvent>
    private let continuation: AsyncStream<VersoChatTransportEvent>.Continuation

    init(
        historyResponses: [VersoChatHistoryPayload],
        sessionsResponses: [VersoChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<VersoChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> VersoChatHistoryPayload {
        ...
        return self.historyResponses.last ?? VersoChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [VersoChatAttachmentPayload]) async throws -> VersoChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return VersoChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> VersoChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? VersoChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: VersoChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = VersoChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = VersoChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { VersoChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = VersoChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = VersoChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = VersoChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { VersoChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
