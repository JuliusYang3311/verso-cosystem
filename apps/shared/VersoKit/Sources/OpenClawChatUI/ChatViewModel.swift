...
private let chatUILogger = Logger(subsystem: "ai.openclaw", category: "VersoChatUI")

@MainActor
@Observable
public final class VersoChatViewModel {
    public private(set) var messages: [VersoChatMessage] = []
    ...
    public var attachments: [VersoPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [VersoChatPendingToolCall] = []
    public private(set) var sessions: [VersoChatSessionEntry] = []
    private let transport: any VersoChatTransport
    ...
    private var pendingToolCallsById: [String: VersoChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any VersoChatTransport) {
    ...
    public var sessionChoices: [VersoChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [VersoChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [VersoChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: VersoPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [VersoChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: VersoChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [VersoChatMessage]) -> [VersoChatMessage] {
        var result: [VersoChatMessage] = []
    ...
    private static func dedupeKey(for message: VersoChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [VersoChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> VersoChatAttachmentPayload in
            VersoChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> VersoChatSessionEntry {
        VersoChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: VersoChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: VersoChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: VersoAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = VersoChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> VersoPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
