...
import "@a2ui/lit/ui";
import { themeContext } from "@verso/a2ui-theme-context";
...
class VersoA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--verso-a2ui-inset-top, 0px)
        var(--verso-a2ui-inset-right, 0px)
        var(--verso-a2ui-inset-bottom, 0px)
        var(--verso-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--verso-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--verso-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--verso-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--verso-a2ui-empty-top, var(--verso-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.versoA2UI = api;
    globalThis.versoA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    globalThis.addEventListener("verso:a2ui-action-status", this.#statusListener);
    this.#syncSurfaces();
  }
  ...
  disconnectedCallback() {
    ...
    if (this.#statusListener) {
      globalThis.removeEventListener("verso:a2ui-action-status", this.#statusListener);
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__versoLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.versoCanvasA2UIAction ??
      globalThis.webkit?.messageHandlers?.versoCanvasA2UIAction ??
      globalThis.versoCanvasA2UIAction ??
      globalThis.versoCanvasA2UIAction;
...
customElements.define("verso-a2ui-host", VersoA2UIHost);
