...
@MainActor
struct VersoChatComposer: View {
    @Bindable var viewModel: VersoChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(VersoChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(VersoChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \VersoPendingAttachment.id)
                { (att: VersoPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            VersoPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(VersoChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(VersoChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(VersoChatTheme.subtleCard)
        .clipShape(Capsule())
    }
