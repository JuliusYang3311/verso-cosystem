---
summary: "Use Venice AI privacy-focused models in Verso"
read_when:
  - You want privacy-focused inference in Verso
...
```bash
verso onboard --auth-choice venice-api-key
```
...
```bash
verso chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Verso shows all available Venice models. Pick based on your needs:
...
```bash
verso models set venice/claude-opus-45
verso models set venice/llama-3.3-70b
```

List all available models:

```bash
verso models list | grep venice
```

## Configure via `verso configure`

1. Run `verso configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Verso automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
verso chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
verso chat --model venice/claude-opus-45

# Use uncensored model
verso chat --model venice/venice-uncensored

# Use vision model with image
verso chat --model venice/qwen3-vl-235b-a22b

# Use coding model
verso chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `verso models list` to see currently available models. Some models may be temporarily offline.
