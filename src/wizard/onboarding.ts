...
export async function runOnboardingWizard(
  ...
  await prompter.intro("Verso onboarding");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: VersoConfig = snapshot.valid ? snapshot.config : {};
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("verso doctor")}\` to repair it, then re-run onboarding.`,
    );
    runtime.exit(1);
    return;
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("verso configure")}.`;
  ...
  // Optional context/compaction config (advanced flow only)
  if (flow === "advanced") {
    const wantContextConfig = await prompter.confirm({
      message: "Configure context window and compaction settings?",
      initialValue: false,
    });
