...
function decoratedMarkerPath(userDataDir) {
  return path.join(userDataDir, ".verso-profile-decorated");
}
 ...
 */
export function decorateVersoProfile(userDataDir, opts) {
  const desiredName = opts?.name ?? DEFAULT_VERSO_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_VERSO_BROWSER_COLOR).toUpperCase();
