...
import {
  ...
  DEFAULT_VERSO_BROWSER_ENABLED,
  DEFAULT_BROWSER_EVALUATE_ENABLED,
  DEFAULT_BROWSER_DEFAULT_PROFILE_NAME,
  DEFAULT_VERSO_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
function normalizeHexColor(raw) {
  ...
  if (!value) {
    return DEFAULT_VERSO_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "verso" profile exists in the profiles map.
...
function ensureDefaultProfile(profiles, defaultColor, legacyCdpPort, derivedDefaultCdpPort) {
  ...
  if (!result[DEFAULT_VERSO_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_VERSO_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Verso browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(profiles, controlPort) {
  ...
  // (legacy single-profile configs may use controlPort+1 for verso/verso CDP).
...
export function resolveBrowserConfig(cfg, rootConfig) {
  const enabled = cfg?.enabled ?? DEFAULT_VERSO_BROWSER_ENABLED;
...
export function resolveProfile(resolved, profileName) {
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "verso";
  if (rawProfileUrl) {
