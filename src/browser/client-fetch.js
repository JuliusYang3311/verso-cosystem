...
function enhanceBrowserFetchError(url, err, timeoutMs) {
  const hint = isAbsoluteHttp(url)
    ...
    : `Start (or restart) the Verso gateway (Verso.app menubar, or \`${formatCliCommand("verso gateway")}\`) and try again.`;
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the verso browser control service (timed out after ${timeoutMs}ms). ${hint}`,
    );
  }
  return new Error(`Can't reach the verso browser control service. ${hint} (${msg})`);
}
