...
export function isUnhandledRejectionHandled(reason) {
  for (const handler of handlers) {
    try {
      ...
      console.error(
        "[verso] Unhandled rejection handler failed:",
...
export function installUnhandledRejectionHandler() {
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    if (isAbortError(reason)) {
      console.warn("[verso] Suppressed AbortError:", formatUncaughtError(reason));
      return;
    }
    if (isFatalError(reason)) {
      console.error("[verso] FATAL unhandled rejection:", formatUncaughtError(reason));
      process.exit(1);
      return;
    }
    if (isConfigError(reason)) {
      console.error("[verso] CONFIGURATION ERROR - requires fix:", formatUncaughtError(reason));
      process.exit(1);
      return;
    }
    if (isTransientNetworkError(reason)) {
      console.warn(
        "[verso] Non-fatal unhandled rejection (continuing):",
        formatUncaughtError(reason),
      );
      return;
    }
    console.error("[verso] Unhandled promise rejection:", formatUncaughtError(reason));
    process.exit(1);
  });
}
