...
function isCronRunSessionKey(key) {
...
export async function pruneAndLoadCombinedSessionStoreForGateway(cfg) {
  const { pruneStaleSessions } = await import("../sessions/persistence.js");
  const storeConfig = cfg.session?.store;
  if (storeConfig && !isStorePathTemplate(storeConfig)) {
    // Single store: prune once
    const defaultAgentId = normalizeAgentId(resolveDefaultAgentId(cfg));
    await pruneStaleSessions({ cfg, agentId: defaultAgentId });
  } else {
    // Multi-store: prune all
    const agentIds = listConfiguredAgentIds(cfg);
    for (const agentId of agentIds) {
      await pruneStaleSessions({ cfg, agentId });
    }
  }
  // Then load standard
  return loadCombinedSessionStoreForGateway(cfg);
}
