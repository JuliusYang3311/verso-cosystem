...
function enqueueOrchestration(item) {
 ...
 */
export async function startOrchestratorDaemon(opts) {
  const cfg = opts.cfg;
 ...
 */
export async function stopOrchestratorDaemon(orchestrationId) {
  const pidPath = resolvePidPath(orchestrationId);
  const pid = readPid(pidPath);
  if (!pid) {
 ...
 */
export async function submitOrchestration(userPrompt, opts) {
  const cfg = opts?.cfg;
  const agentId = opts?.agentId ?? "main";
  const workspace = resolveWorkspace(cfg);
