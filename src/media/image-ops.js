...
function prefersSips() {
  return (
    process.env.VERSO_IMAGE_BACKEND === "sips" ||
    (process.env.VERSO_IMAGE_BACKEND !== "sharp" && isBun() && process.platform === "darwin")
  );
}
...
async function withTempDir(fn) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "verso-img-"));
  try {
    return await fn(dir);
  } finally {
