...
export async function resolveReplyDirectives(params) {
  ...
  const resolvedThinkLevel =
    directives.thinkLevel ?? sessionEntry?.thinkingLevel ?? agentCfg?.thinkingDefault;
  const resolvedVerboseLevel =
    directives.verboseLevel ?? sessionEntry?.verboseLevel ?? agentCfg?.verboseDefault;
  const resolvedReasoningLevel = directives.reasoningLevel ?? sessionEntry?.reasoningLevel ?? "off";
  const resolvedElevatedLevel = elevatedAllowed
    ? (directives.elevatedLevel ?? sessionEntry?.elevatedLevel ?? agentCfg?.elevatedDefault ?? "on")
    : "off";
