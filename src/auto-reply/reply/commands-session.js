...
import { scheduleGatewaySigusr1Restart, triggerVersoRestart } from "../../infra/restart.js";
...
export const handleRestartCommand = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Verso in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerVersoRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Verso via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
