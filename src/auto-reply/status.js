...
export const formatContextUsageShort = (total, contextTokens) =>
...
const readUsageFromSessionLog = (sessionId, sessionEntry) => {
  // Transcripts are stored at the session file path (fallback: ~/.verso/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
...
export function buildStatusMessage(args) {
  ...
  const versionLine = `🦞 Verso ${VERSION}${commit ? ` (${commit})` : ""}`;
