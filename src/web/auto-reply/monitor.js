...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    const statusCode =
      (typeof reason === "object" && reason && "status" in reason ? reason.status : undefined) ??
      "unknown";
    const loggedOut =
      typeof reason === "object" && reason && "isLoggedOut" in reason && reason.isLoggedOut;
    const errorStr = formatError(reason);
    ...
    if (loggedOut) {
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("verso channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
