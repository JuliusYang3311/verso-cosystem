    ...
    if (isGif) {
      result = await sendWithThreadFallback(
        ...
        async (effectiveParams, label) =>
          requestWithDiag(() => api.sendAnimation(chatId, file, effectiveParams), label).catch(
            (err) => {
              throw wrapChatNotFound(err);
            },
          ),
      ...
      result = await sendWithThreadFallback(mediaParams, "photo", async (effectiveParams, label) =>
        requestWithDiag(() => api.sendPhoto(chatId, file, effectiveParams), label).catch((err) => {
          throw wrapChatNotFound(err);
        }),
      );
    } else if (kind === "video") {
      if (isVideoNote) {
        result = await sendWithThreadFallback(
          ...
          "video_note",
          async (effectiveParams, label) =>
            requestWithDiag(() => api.sendVideoNote(chatId, file, effectiveParams), label).catch(
              (err) => {
                throw wrapChatNotFound(err);
              },
            ),
        ...
        result = await sendWithThreadFallback(
          ...
          "video",
          async (effectiveParams, label) =>
            requestWithDiag(() => api.sendVideo(chatId, file, effectiveParams), label).catch(
              (err) => {
                throw wrapChatNotFound(err);
              },
            ),
      ...
      if (useVoice) {
        result = await sendWithThreadFallback(
          ...
          "voice",
          async (effectiveParams, label) =>
            requestWithDiag(() => api.sendVoice(chatId, file, effectiveParams), label).catch(
              (err) => {
                throw wrapChatNotFound(err);
              },
            ),
        ...
        result = await sendWithThreadFallback(
          ...
          "audio",
          async (effectiveParams, label) =>
            requestWithDiag(() => api.sendAudio(chatId, file, effectiveParams), label).catch(
              (err) => {
                throw wrapChatNotFound(err);
              },
            ),
      ...
      result = await sendWithThreadFallback(
        ...
        "document",
        async (effectiveParams, label) =>
          requestWithDiag(() => api.sendDocument(chatId, file, effectiveParams), label).catch(
            (err) => {
              throw wrapChatNotFound(err);
            },
          ),
 ...
 */
export async function sendMessageDraftTelegram(chatId, draftId, text, opts = {}) {
  const cfg = loadConfig();
  const account = resolveTelegramAccount({
    cfg,
    accountId: opts.accountId,
  ...
  await requestWithDiag(() => api.sendMessageDraft(draftParams), "sendMessageDraft").catch(
    async (err) => {
      // Telegram rejects malformed HTML. Fall back to plain text.
      const errText = formatErrorMessage(err);
      if (PARSE_ERR_RE.test(errText)) {
        ...
        return await requestWithDiag(
          () => api.sendMessageDraft(plainParams),
