...
function deriveIdHint(params) {
  ...
  // package is scoped (example: @verso/voice-call -> voice-call).
...
function addCandidate(params) {
  ...
  params.candidates.push({
    ...
    packageVerso: getPackageManifestMetadata(manifest ?? undefined),
  });
}
...
export function discoverVersoPlugins(params) {
  const candidates = [];
  const diagnostics = [];
  const seen = new Set();
  ...
  if (workspaceDir) {
    ...
    const workspaceExt = path.join(workspaceRoot, ".verso", "extensions");
    discoverInDirectory({
      dir: workspaceExt,
      origin: "workspace",
