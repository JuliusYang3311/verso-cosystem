...
import { discoverVersoPlugins } from "./discovery.js";
import { loadPluginManifest } from "./manifest.js";
const registryCache = new Map();
const DEFAULT_MANIFEST_CACHE_MS = 200;
function resolveManifestCacheMs(env) {
  const raw = env.VERSO_PLUGIN_MANIFEST_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseManifestCache(env) {
  const disabled = env.VERSO_DISABLE_PLUGIN_MANIFEST_CACHE?.trim();
  if (disabled) {
    return false;
  }
