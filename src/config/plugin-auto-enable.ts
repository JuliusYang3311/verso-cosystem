...
function resolveChannelConfig(cfg: VersoConfig, channelId: string): Record<string, unknown> | null {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  const entry = channels?.[channelId];
  return isRecord(entry) ? entry : null;
}

function isTelegramConfigured(cfg: VersoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: VersoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: VersoConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: VersoConfig): boolean {
...
function isIMessageConfigured(cfg: VersoConfig): boolean {
...
function isWhatsAppConfigured(cfg: VersoConfig): boolean {
...
function isGenericChannelConfigured(cfg: VersoConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: VersoConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: VersoConfig, providerId: string): boolean {
...
function resolveConfiguredPlugins(cfg: VersoConfig, env: NodeJS.ProcessEnv): PluginEnableChange[] {
...
function isPluginExplicitlyDisabled(cfg: VersoConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: VersoConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: VersoConfig, pluginId: string): VersoConfig {
...
function registerPluginEntry(cfg: VersoConfig, pluginId: string): VersoConfig {
