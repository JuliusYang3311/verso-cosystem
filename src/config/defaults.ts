...
const DEFAULT_MODEL_ALIASES: Readonly<Record<string, string>> = {
  ...
  // Google Gemini (3.1 are preview ids in the catalog)
...
function resolveAnthropicDefaultAuthMode(cfg: VersoConfig): AnthropicAuthDefaultsMode | null {
...
export function applyMessageDefaults(cfg: VersoConfig): VersoConfig {
...
export function applyTalkApiKey(config: VersoConfig): VersoConfig {
...
export function applyModelDefaults(cfg: VersoConfig): VersoConfig {
  let mutated = false;
  let nextCfg = cfg;
...
export function applyAgentDefaults(cfg: VersoConfig): VersoConfig {
...
export function applyLoggingDefaults(cfg: VersoConfig): VersoConfig {
...
export function applyContextPruningDefaults(cfg: VersoConfig): VersoConfig {
...
export function applyCompactionDefaults(cfg: VersoConfig): VersoConfig {
  const defaults = cfg.agents?.defaults;
  if (!defaults) {
    return cfg;
  }
