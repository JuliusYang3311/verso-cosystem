...
interface BuildGepPromptParams {
  nowIso: string;
  context: string;
  signals: unknown;
  selector: unknown;
  ...
  capsuleCandidates: Array<{ id?: string }> | null;
  genesPreview: string;
  capsulesPreview: string;
  capabilityCandidatesPreview: string | null;
  externalCandidatesPreview: string | null;
}

export function buildGepPrompt({
  nowIso,
  context,
  signals,
  selector,
...
6. KNOWN ISSUES (DO NOT ATTEMPT TO FIX -- already handled externally)
   ...
   - "ENOENT" / "spawn verso" -- PATH resolution issue, fixed in wrapper with explicit binary search.
