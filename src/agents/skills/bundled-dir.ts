...
import { resolveVersoPackageRootSync } from "../../infra/openclaw-root.js";
...
export function resolveBundledSkillsDir(
  ...
  const override = process.env.VERSO_BUNDLED_SKILLS_DIR?.trim();
  if (override) {
    return override;
  }
  ...
  try {
    ...
    const packageRoot = resolveVersoPackageRootSync({
      argv1,
      moduleUrl,
      cwd,
    });
