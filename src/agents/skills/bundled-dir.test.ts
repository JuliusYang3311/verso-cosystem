...
describe("resolveBundledSkillsDir", () => {
  const originalOverride = process.env.VERSO_BUNDLED_SKILLS_DIR;

  afterEach(() => {
    if (originalOverride === undefined) {
      delete process.env.VERSO_BUNDLED_SKILLS_DIR;
    } else {
      process.env.VERSO_BUNDLED_SKILLS_DIR = originalOverride;
    }
  });
