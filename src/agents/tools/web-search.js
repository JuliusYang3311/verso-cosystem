...
// ---------- URL deduplication ----------
export function deduplicateWebResults(results) {
  const map = new Map();
...
function snippetSimilarity(a, b) {
  ...
  const text = (r) => `${r.title} ${r.description}`;
...
function resultTokens(r) {
  return Math.ceil(`${r.title} ${r.description}`.length / 4);
}
export function mmrSelectWebResults(candidates, lambda, minGain, budgetTokens) {
...
// ---------- Per-factor count ----------
export function perFactorCount(targetTotal, factorCount) {
  return factorCount <= 0 ? targetTotal : Math.ceil(targetTotal / factorCount);
}
function resolveSubqueries(space, query, mmrLambda, queryVec = []) {
