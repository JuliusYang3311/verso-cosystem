...
JOB SCHEMA (for add action):
{
  ...
  "delivery": { ... },      // Optional, auto-detected from session route
...
DELIVERY (isolated-only, auto-detected):
  Delivery target is automatically inferred from the current session's inbound route.
  For isolated agentTurn jobs, completion summaries are announced back to the originating channel (e.g. Telegram, WhatsApp).
  You do NOT need to set delivery — it is handled automatically. Do not call messaging tools inside the run.
        ...
        case "add": {
          ...
          if (
            ...
            if (shouldInfer) {
              const cfg = loadConfig();
              const agentId =
                resolveSessionAgentId({ sessionKey: opts.agentSessionKey, config: cfg }) ?? "main";
              const detected = await autoDetectDelivery(cfg, agentId, opts.agentSessionKey);
              if (detected) {
                const inferred: CronDelivery = { mode: "announce", to: detected.to };
                if (detected.channel) {
                  inferred.channel = detected.channel as CronMessageChannel;
                }
