        ...
        while (true) {
          ...
          if (imageDimensionError && lastProfileId) {
            const details = [
              ...
              .join(" ");
            log.warn(
              `Profile ${lastProfileId ?? "unknown"} rejected image payload${details ? ` (${details})` : ""}.`,
            );
          }
          ...
          if (shouldRotate) {
            if (lastProfileId) {
              ...
              if (timedOut && !isProbeSession) {
                log.warn(
                  `Profile ${lastProfileId ?? "unknown"} timed out (possible rate limit). Trying next account...`,
                );
              }
              if (cloudCodeAssistFormatError) {
                log.warn(
                  `Profile ${lastProfileId ?? "unknown"} hit Cloud Code Assist format error. Tool calls will be sanitized on retry.`,
                );
              }
            }
