...
export function buildSandboxCreateArgs(params) {
  ...
  args.push("--label", "verso.sandbox=1");
  args.push("--label", `verso.sessionKey=${params.scopeKey}`);
  args.push("--label", `verso.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `verso.configHash=${params.configHash}`);
  }
  for (const [key, value] of Object.entries(params.labels ?? {})) {
    if (key && value) {
      ...
      args.push("--label", `${key}=${stringValue}`);
    }
  }
...
async function readContainerConfigHash(containerName) {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "verso.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) {
    return null;
...
function formatSandboxRecreateHint(params) {
  if (params.scope === "session") {
    return formatCliCommand(`verso sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`verso sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("verso sandbox recreate --all");
}
