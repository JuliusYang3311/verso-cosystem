...
function buildSandboxBrowserResolvedConfig(params) {
  ...
  return {
    ...
    color: DEFAULT_VERSO_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: "verso",
    profiles: {
      verso: { cdpPort: params.cdpPort, color: DEFAULT_VERSO_BROWSER_COLOR },
    },
  };
}
...
export async function ensureSandboxBrowser(params) {
  ...
  if (!state.exists) {
    ...
    args.push("-e", `VERSO_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `VERSO_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `VERSO_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `VERSO_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `VERSO_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
  ...
  const existingProfile = existing ? resolveProfile(existing.bridge.state.resolved, "verso") : null;
  const shouldReuse =
