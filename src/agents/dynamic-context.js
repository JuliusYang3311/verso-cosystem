 ...
 */
export async function loadContextParams() {
  try {
 ...
 */
export function computeDynamicRecentRatio(recentMessages, params) {
  ...
  const toolResultCount = recentMessages
    .slice(-10)
    .filter((m) => m && typeof m === "object" && m.role === "toolResult").length;
 ...
 */
export function timeDecayFactor(timestampMs, lambda) {
 ...
 */
export function selectRecentMessages(allMessages, budgetTokens) {
...
function fromDiverseChunk(chunk) {
  return {
    path: chunk.path,
    startLine: chunk.startLine,
    endLine: chunk.endLine,
 ...
 */
export function filterRetrievedChunks(chunks, params, budgetTokens) {
  if (chunks.length === 0) {
...
function estimateSnippetTokens(text) {
 ...
 */
export function progressiveLoadChunks(chunks, params, budgetTokens) {
